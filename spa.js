...
function viewHome() {
    ...
    let html = `
        ...
        <div class="px-5 mb-9">
            <div class="relative h-64 rounded-3xl overflow-hidden shadow-xl shadow-black/5 active:scale-[0.98] transition-all">
                <img src="${produtos.length > 0 ? produtos[0].imagem : 'https://images.unsplash.com/photo-1483985988355-763728e1935b?w=1000&q=80'}" alt="Banner" class="w-full h-full object-cover">
                <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent z-10"></div>
                <div class="absolute bottom-5 left-6 right-6 text-white z-20">
                    <h1 class="text-xl font-black tracking-tight mb-0.5 leading-tight">${lojaDisplayTitle}</h1>
                    <p class="text-[13px] font-semibold text-slate-200 leading-relaxed">Nova coleção disponível.</p>
                </div>
            </div>
        </div>
                ...
                <div onclick="navegarPara('produto', '${p.id}')" class="cursor-pointer group flex flex-col">
                    ...
                    <div class="px-1 flex flex-col gap-1 mt-1">
                        ...
                        <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                            ...
                            <p class="text-[15px] font-black ${p.precoOriginal && p.preco < p.precoOriginal ? 'text-emerald-500' : 'text-slate-900'} tracking-tight">${p.preco.toLocaleString('pt-MZ')} <span class="text-[10px] ${p.precoOriginal && p.preco < p.precoOriginal ? 'text-emerald-400' : 'text-slate-400'} font-bold">MT</span></p>
                        </div>
                    </div>
                </div>
            `;
    ...
    return `
        <div onclick="navegarPara('produto', '${p.id}')" class="cursor-pointer group flex-shrink-0 w-[170px] snap-start">
            ...
            <div class="px-1 flex flex-col gap-1 mt-1">
                ...
                <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                    ${p.precoOriginal && p.preco < p.precoOriginal ? `<span class="text-[10px] text-slate-400 line-through">${p.precoOriginal.toLocaleString('pt-MZ')} MT</span>` : ''}
                    <p class="text-[14px] font-black ${p.precoOriginal && p.preco < p.precoOriginal ? 'text-emerald-500' : 'text-slate-900'} tracking-tight">${p.preco.toLocaleString('pt-MZ')} <span class="text-[9px] ${p.precoOriginal && p.preco < p.precoOriginal ? 'text-emerald-400' : 'text-slate-400'} font-bold">MT</span></p>
                </div>
            </div>
        </div>
    `;
            ...
            <div class="bg-white rounded-t-3xl -mt-5 relative z-30 px-5 pt-6 pb-8 flex-shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] flex flex-col gap-4 border-t border-slate-100">
                <div class="flex justify-between items-start gap-4">
                    <div class="flex flex-col">
                        ...
                        <h1 class="text-base font-black text-slate-900 leading-tight line-clamp-2">${p.nome}</h1>
                    </div>
                    <div class="text-xl font-black text-slate-900 text-right whitespace-nowrap pt-1 flex flex-col items-end">
                        ${p.precoOriginal && p.preco < p.precoOriginal ? `<span class="text-[9px] text-slate-400 line-through">${p.precoOriginal.toLocaleString('pt-MZ')} MT</span>` : ''}
                        <div>${p.preco.toLocaleString('pt-MZ')} <span class="text-[10px] font-bold text-slate-400">MT</span></div>
                    </div>
                </div>
    ...
    resultados.forEach(p => {
        html += `
            <div onclick="fecharPesquisa(); navegarPara('produto', '${p.id}')" class="cursor-pointer group flex flex-col">
                ...
                <div class="px-1">
                    <h3 class="text-[12px] font-semibold text-slate-500 truncate">${p.nome}</h3>
                    <p class="text-[14px] font-black text-slate-900">${p.preco.toLocaleString('pt-MZ')} MT</p>
                </div>
            </div>
        `;
    });
